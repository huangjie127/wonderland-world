...
1. 复制上面的整个脚本（从第一个 DROP 到最后）
2. 粘贴到 SQL 编辑器
3. 点击 **"Run"** 按钮（或按 Ctrl+Enter）
4. 等待完成（应该显示 "Success"）

## ⚠️ 第四步：创建 Storage Bucket（必须做！）

**重要：如果不做这一步，创建角色时会报错 "bucket not found"**

1. 进入 Supabase 控制台：https://app.supabase.com
2. 选择你的项目
3. 左侧菜单 → **Storage**
4. 点击 **Create new bucket**
5. 填写信息：
   - **Bucket name**: `avatars`
   - **Public bucket**: ✅ 勾选（这样头像能被公开访问）
6. 点击 **Create bucket**

**验证成功：** Storage 页面应该看到 `avatars` bucket 列在列表中

## 第五步：配置 Storage RLS 策略（可选但推荐）
...
你的 Supabase 已经完全配置好了。现在可以：
1. 注册账号
2. 登录
3. **创建角色并上传头像**（会自动上传到 avatars bucket）
4. 在 Home 页查看你的角色
5. 在 Archive 查看所有角色
...
**如果创建角色时报 "bucket not found"：**
- 检查是否完成了第四步
- 检查 bucket 名字是否完全是 `avatars`（小写）
